...
/**
 * @example mcuxClSession_cleanupOnError_example.c
 * @brief   Example for the mcuxClSession CleanupOnError function.
 */
...
static const uint8_t data[] = {
  0xCAu, 0x83u, 0x5Fu, 0x5Bu, 0x0Bu, 0x44u, 0x33u, 0x32u,
  0x11u, 0x4Eu, 0xEAu, 0xA5u, 0x9Au, 0xE5u, 0x56u, 0x7Eu,
  0x4Fu, 0x86u, 0xAFu, 0x72u, 0xF1u, 0x1Cu, 0xB4u, 0xE6u,
  0x71u, 0xD9u, 0x13u, 0xDEu, 0x03u, 0xD4u, 0xC5u, 0xB7u
};
...
MCUXCLEXAMPLE_FUNCTION(mcuxClSession_cleanupOnError_example)
{
  /**************************************************************************/
  /* Preparation                                                            */
  /**************************************************************************/
  ...
  /**************************************************************************/
  /* Verify with a valid public key                                         */
  /**************************************************************************/
  ...
  /* Signature Verification with a valid public key */
  ...
  /* Check the token and return status and return accordingly */
  if((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClSignature_verify) == sv_token) && (MCUXCLSIGNATURE_STATUS_OK == sv_status))
  {
    /* Signature OK scenario */
    return MCUXCLEXAMPLE_STATUS_OK;
  }
  else if((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClSignature_verify) == sv_token) &&
          (MCUXCLSIGNATURE_STATUS_NOT_OK == sv_status))
  {
    /* Signature NOT OK scenario */
    return MCUXCLEXAMPLE_STATUS_ERROR;
  }
  else if(MCUXCLCORE_CLS_ABNORMAL == MCUXCLCORE_GET_CLS(sv_status))
  {
    /* Error scenario */
    /* Perform cleanuponError and release the used resources */
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(resultCleanup, tokenCleanup, mcuxClSession_cleanupOnError(session));
    if((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClSession_cleanupOnError) != tokenCleanup) ||
       (MCUXCLSESSION_STATUS_OK != resultCleanup))
    {
  ...
  {
    /* Fault scenario */
    /* The cleanuponError function should not be used here, but instead Power-On-Reset or System Reset must be performed. */
    return MCUXCLEXAMPLE_STATUS_ERROR;
  }
