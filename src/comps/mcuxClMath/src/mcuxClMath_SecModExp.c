 ...
 * @param[in] numSqr          chooses the used algorithm, can be 1 (double-exponentiation) or 2 (fixed 2-bit window algorithm)
 *
 * @retval void
 */
...
mcuxClMath_SecModExp_SqrMultAws(const uint32_t *pExp, uint32_t expByteLength, uint32_t numSqr)
{
    ...
    MCUX_CSSL_ANALYSIS_COVERITY_ASSERT_FP_VOID(expByteLength, sizeof(uint32_t), MCUXCLPKC_RAM_SIZE)
    /* ASSERT: numSqr is either 1 or 2. */
    MCUX_CSSL_ANALYSIS_COVERITY_ASSERT_FP_VOID(numSqr, 1u, 2u)

    /* DI protect the square mult loop */
    MCUX_CSSL_DI_RECORD(SqrMultAws_SquMulLoop_LoadExpWord, expByteLength / 4u);
    MCUX_CSSL_DI_RECORD(SqrMultAws_SquMulLoop_Square, expByteLength * 4u * numSqr);
    MCUX_CSSL_DI_RECORD(SqrMultAws_SquMulLoop_OperandRerandomization, 32u);
    ...
    do
    {
        ...
        if (0x1Eu == ((uint32_t) bitIndex & 0x1Fu))
        {
            ...
            /* DI balance the execution of this branch in the loop */
            MCUX_CSSL_DI_EXPUNGE(SqrMultAws_SquMulLoop_LoadExpWord, 1u);

            /* Read one CPU word of exponent and mask it. */
            MCUX_CSSL_FP_FUNCTION_CALL(prngGenerateWordRet1, mcuxClPrng_generate_word());
            expWord1 = prngGenerateWordRet1;
        ...
        if (((int32_t) bitLenExp - 64) <= bitIndex)
        {
            ...
            /* DI balance the execution of this branch in the loop */
            MCUX_CSSL_DI_EXPUNGE(SqrMultAws_SquMulLoop_OperandRerandomization, 1u);
            ...
            * The upper words of buffers R2 and R2H have already been cleared.*/
            ...
            p32R0[0u] = randModBlinding >> 6u; /* R0 = 26-bit random  */
            p32R0[1u] = 0x00000000u;
            MCUX_CSSL_FP_FUNCTION_CALL(prngGenerateWordRet2, mcuxClPrng_generate_word());
            randModBlinding = prngGenerateWordRet2;
            p32R0[2u] = randModBlinding >> 6u; /* R1 = 26-bit random  */
            p32R0[3u] = 0x00000000u;
            MCUX_CSSL_FP_FUNCTION_CALL(prngGenerateWordRet3, mcuxClPrng_generate_word());
            randModBlinding = prngGenerateWordRet3;
            p32R0[4u] = randModBlinding >> 6u; /* R2 = 26-bit random  */
            MCUX_CSSL_FP_FUNCTION_CALL(prngGenerateWordRet4, mcuxClPrng_generate_word());
            randModBlinding = prngGenerateWordRet4;
            p32R0[6u] = randModBlinding >> 6u; /* R2H = 26-bit random  */
        ...
        do
        {
            ...
            /* DI balance the execution of this loop */
            MCUX_CSSL_DI_EXPUNGE(SqrMultAws_SquMulLoop_Square, 1u);
        ...
        MCUX_CSSL_FP_FUNCTION_CALL(prngGenerateWordRet5, mcuxClPrng_generate_word());
        uint32_t rndWord = prngGenerateWordRet5;
...
MCUX_CSSL_FP_PROTECTED_TYPE(void) mcuxClMath_SecModExp(
    mcuxClSession_Handle_t session, uint32_t *pExpTemp, uint32_t expByteLength, uint32_t iT3_iX_iT2_iT1, uint32_t iN_iTE_iT0_iR)
{
    ...
    MCUX_CSSL_ANALYSIS_COVERITY_ASSERT_FP_VOID(expByteLength, 1u, MCUXCLPKC_RAM_SIZE - 1u)

    /* DI protect the exact divide loop */
    MCUX_CSSL_DI_RECORD(SecModExp_ExactDivideLoop, MCUXCLCORE_NUM_OF_WORDS_CEIL(MCUXCLPKC_WORDSIZE, expByteLength + 1u));
    ...
    MCUX_CSSL_FP_FUNCTION_CALL_VOID(mcuxClRandom_ncGenerate_Internal(session, pA1, pkcLenExpPlus));
    ...
    MCUX_CSSL_ANALYSIS_COVERITY_ASSERT_FP_VOID(offsetTE, MCUXCLPKC_RAM_OFFSET_MIN, MCUXCLPKC_RAM_OFFSET_MAX - (6u*MCUXCLPKC_WORDSIZE))
    ...
    if (MCUXCLPKC_IS_PKC_RAM(pExpTemp))
    {
        /* It will be used in the FUP to store the calculation result of q = A0-A1 */
        pOperands[SECMODEXP_EXPT] = MCUXCLPKC_PTR2OFFSET((uint8_t *)pExpTemp);
    }
    else
    {
        /* If pExpTemp points to CPU WA, q will be calculated in A0 and then exported to pExpTemp. */
        pOperands[SECMODEXP_EXPT] = pOperands[SECMODEXP_A0];
    }

    /* Generate a 64-bit random b with both MSbit and LSbit set. */
    uint32_t *p32R0 = MCUXCLPKC_OFFSET2PTRWORD((uint16_t) offsetR0);  /* operand R0 shall be PKC word aligned. */
    MCUXCLPKC_PKC_CPU_ARBITRATION_WORKAROUND();  // avoid CPU accessing to PKC workarea when PKC is busy
    uint32_t rand32B = mcuxClRandom_ncGenerateWord_Internal(session);
    p32R0[0u] = rand32B | 0x00000001u;
    rand32B = mcuxClRandom_ncGenerateWord_Internal(session);
    p32R0[1u] = rand32B | 0x80000000u;
...
//  MCUXCLPKC_WAITFORREADY();  <== there is WAITFORREADY in QDASH
    ...
    MCUX_CSSL_ANALYSIS_COVERITY_ASSERT_FP_VOID(ofsV1_ofsV0, ((uint32_t) MCUXCLPKC_RAM_OFFSET_MIN << 16u) + MCUXCLPKC_RAM_OFFSET_MIN,
    ...
    do
    {
        ...
        MCUX_CSSL_DI_EXPUNGE(SecModExp_ExactDivideLoop, 1u);
    ...
    /* FUP program:
     * 1)  EXPT = q = A0 - A1 = ((expA - expB) - ((expA - expB) mod b)) / b = (exp - (exp mod b)) / b.
    ...
    /* DI protect wordLenExp - will be balanced by call to mcuxClMath_SecModExp_SqrMultAws() */
    MCUX_CSSL_DI_RECORD(SecModExp_wordLenExp, wordLenExp);
    ...
    /* DI balance the parameter expByteLength after its last usage - will be RECORDed by callers */
    MCUX_CSSL_DI_EXPUNGE(SecModExp_expByteLength, expByteLength);
    ...
    p32R0[3u] = 0x00000000u;
    MCUX_CSSL_FP_FUNCTION_CALL(randWord, mcuxClPrng_generate_word());
    p32R0[2u] = randWord >> 16u;
    ...
    MCUX_CSSL_FP_FUNCTION_CALL_VOID(mcuxClMath_SecModExp_SqrMultAws(pExpTemp, wordLenExp, 2u));
    ...
    /* DI protect the length of the exponent for the call to mcuxClMath_SecModExp_SqrMultAws() */
    MCUX_CSSL_DI_RECORD(SecModExp_exponentDoubleExp, 16u);
    MCUX_CSSL_FP_FUNCTION_CALL_VOID(mcuxClMath_SecModExp_SqrMultAws(p32TE, 16u, 1u));
    ...
    MCUX_CSSL_FP_FUNCTION_EXIT_VOID(mcuxClMath_SecModExp,
