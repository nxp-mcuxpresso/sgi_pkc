...
/**
 * @example mcuxClHashModes_sha256_oneshot_dma_nonBlocking_example.c
...
{
  (void)data;
  sha2APInonBlockingStatus_callback = status;
}
...
uint8_t longData[] = {
  ...
  MCUXCLEXAMPLE_ALLOCATE_AND_INITIALIZE_SESSION(session, MCUXCLEXAMPLE_MAX_WA(MCUXCLHASH_COMPUTE_NONBLOCKING_CPU_WA_BUFFER_SIZE_SHA2_256, MCUXCLRANDOM_NCINIT_WACPU_SIZE), 0u);
  ...
  MCUXCLBUFFER_INIT_DMA_RO(longDataBuf, session, longData, sizeof(longData));
  ...
  /* mcuxCl_InputBuffer_t pIn:       */ longDataBuf,
  /* uint32_t inSize:               */ sizeof(longData),
  ...
  if(MCUXCLHASH_STATUS_JOB_STARTED == result)
  {
    ...
    if(MCUXCLHASH_STATUS_JOB_COMPLETED != sha2APInonBlockingStatus_callback)
    {
      return MCUXCLEXAMPLE_STATUS_ERROR;
    }
