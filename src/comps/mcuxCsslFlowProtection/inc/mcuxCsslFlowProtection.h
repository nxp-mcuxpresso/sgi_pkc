 ...
 *
 *
 * The flow protection mechanism is based on events and expectations that need to be balanced.
 * Every check that an event has occured should correspond to an expectation that has been raised.
 *
 * This is a common API with various backends that implement its functionality.
 * The backends can be divided into the following classes:
 *
 * @subsection None
 *
 * The None backend allows the use of flow protection macros on platforms that do not support any of the regular backends.
 * It allows portability of code including flow protection macros, without implementing any protection mechanims.
 *
 * @subsection Secure Counter
 *
 * Security Counter based backends allow a check that the execution matches the general expected code flow,
 * but do not provide protection against individual instruction skips within a part of linear code flow.
 *
 * There are two Secure Counter backends, called Local and Global.
 * Local and Global backends implement functionality differently but are to be used in the same way.
 * Both are based on the Security Counter and rely on the matching Security Counter backend to be active for the platform.
 *
 * For the current Global implementation, users can omit MCUX_CSSL_FP_PROTECTED_TYPE,
 *    MCUX_CSSL_FP_FUNCTION_CALL, MCUX_CSSL_FP_FUNCTION_CALL_VOID macros, as they resolve
 *    to standard unprotected code you would use without these macros.
 *    (This might change in the future.)
 *
 * @subsection Code Signature
 *
 * The CodeSignature backend allows protection against individual instruction skips in non-linear and linear code.
 *
 * The set of macros useful for the CodeSignature backend is disjoint from the set of useful
 * macros for Local / Global backends.
 *
 * Only MCUX_CSSL_FP_FUNCTION_DECL, MCUX_CSSL_FP_FUNCTION_DEF, and MCUX_CSSL_FP_FUNCTION_POINTER have
 * an impact for CodeSignature. Whereas, for Local and Global backends, these three macros do not
 * have any impact.
 */
 ...
 * @ingroup csslFpCore
 *
 * This macro can be applied to create or use supporting variables that are only needed if
 ...
 * @ingroup mcuxCsslFlowProtection
 *
 * This subset of Flow protection macros is used to protect function calls.
 *
 * @note
 * Each function call using MCUX_CSSL_FP_FUNCTION_CALL needs to be balanced with MCUX_CSSL_FP_FUNCTION_CALLED.
 * If the same function is called several times from within the same function scope, MCUX_CSSL_FP_FUNCTION_CALLED
 * needs to be used as many times.
 *
 * @see @ref mcuxCsslFlowProtection_Function_example.c
 *
 ...
 *                  This should be a normal status code.
 *        - expect: Zero or more (comma separated) declarations of expected code
 *               flow behavior.
 ...
 * @ingroup mcuxCsslFlowProtection
 *
 * This subset of Flow protection macros is used to protect any kind of loops (for, do while , while).
 *
 * @see @ref mcuxCsslFlowProtection_Loop_example.c
 *
 ...
 * Important note:
 *
 * For this protection to work, both if and else cases need to be present, even if empty besides the
 * MCUX_CSSL_FP_BRANCH_POSITIVE or MCUX_CSSL_FP_BRANCH_NEGATIVE macro usage.
 *
 * Both cases need either MCUX_CSSL_FP_BRANCH_POSITIVE or MCUX_CSSL_FP_BRANCH_NEGATIVE (not both, not none).
 *
 * Every MCUX_CSSL_FP_BRANCH_POSITIVE macro use needs to be balanced with MCUX_CSSL_FP_BRANCH_TAKEN_POSITIVE,
 * specifying a matching condition. (Same for MCUX_CSSL_FP_BRANCH_NEGATIVE with MCUX_CSSL_FP_BRANCH_TAKEN_NEGATIVE).
 *
 * @see @ref mcuxCsslFlowProtection_Branch_example.c
 *
 ...
 * This branch event macro informs the flow mechanism that the positive scenario
 * of the branch is executed for the branch declared by
 * #MCUX_CSSL_FP_BRANCH_DECL with the given @p id.
 *
 ...
 * This expectation macro indicates to the flow protection mechanism that the
 * branch declared by #MCUX_CSSL_FP_BRANCH_DECL with the given @p id has
 * executed the negative scenario (under the given @p condition).
 *
 ...
 * @ingroup mcuxCsslFlowProtection
 *
 * This subset of Flow protection macros is used to protect switch case statements.
 *
 * @see @ref mcuxCsslFlowProtection_Branch_example.c
 *
 ...
 *
 * This subset of Flow protection macros is used to formulate and verify FP expectations within a function body.
 *
 * @ingroup mcuxCsslFlowProtection
 */
 ...
 * @see MCUX_CSSL_FP_EXPECT
 *
 * @see @ref mcuxCsslFlowProtection_Assert_example.c
 *
 * @param ...    The following parameters need to be passed (comma separated):
 *        - expect: One or more (comma separated) declarations of expected code
 *                  flow behavior.
